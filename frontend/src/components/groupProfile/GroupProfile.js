...
const GroupProfile = () => {
  ...
  const handelAddMember = async (e) => {
    ...
    if (groupMembers.includes(e.target.value))
      alert("Friend Is Already Exists To This Group");
    else {
      if (
        window.confirm(
        ...
        try {
          const response = await axios.put(
            "api/v1/addMember/" + otherName + "/" + e.target.value
          );
  ...
  const deleteMember = async (member) => {
    if (
      ...
      try {
        const response = await axios.put(
          "/api/v1/editgroupmember/" + otherName + "/" + member
        );
        if (response.status === 200) {
          alert(
  ...
  if (addMembers) {
    return (
      <div className="AddMemberDiv">
        <h2>Click On The Friend You Want to Add In This Group</h2>
        {conv.map((elm) => {
          return (
            <button
              className="friends"
